...
class Hang:
    def __init__(self):
        self.equipment_probabilities = {
            ...
            '실버': {'일반': 60, '고급': 28, '희귀': 10.5, '영웅': 1.5},
            '골드': {'일반': 55, '고급': 30, '희귀': 12, '영웅': 3}
        }
        
        self.equipment_type = {'무기': 30, '방어구': 60, '악세사리': 10}
                ...
                if selection in self.equipment_probabilities:
                    ...
                    while equipment_grade is None:
                        ...
                        if equipment_grade is None:
                            print('확률 범위에 해당하는 장비 등급이 없습니다. 다시 시도합니다.')
                            percent = random.randint(1, 100)
                    
                    # 장비 종류 선택
                    equipment_type = random.choices(list(self.equipment_type.keys()), 
                                                    weights=list(self.equipment_type.values()))[0]
                        
                    print(f'{equipment_grade} - {equipment_type} 항아리 뽑기 결과는!!!!!! 입니다!\n\n')
                elif selection == '그만':
                    print('게임을 종료합니다.')
                    break
                else:
...
print(json.dumps(rrr.replace("                    ","").replace("            ","").replace("\n","").replace('    ','')))
